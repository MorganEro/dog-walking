        ...
        if (itemClicked.id.startsWith("walker")) {
           ...
           for (const walker of walkers) {
                if (walker.id === parseInt(walkerId)){
                    const getClickedWalkersCitiesArray =() => {   
                        const clickedWalkersCities = []
                        for (const walkerCity of walkerCities){
                            if (walker.id === walkerCity.walkerId){
                                clickedWalkersCities.push(walkerCity)
                            }
                        }    
                        return clickedWalkersCities
                    }  
                    const arrayOfClickedWalkersCities = getClickedWalkersCitiesArray()

                    const getClickedWalkersCityNames = (walkersCitiesArray) => {
                        let cityNames = ""
                        for (const walkerCity of walkersCitiesArray){
                    ...
                    const cityNamesForClickedWalker = getClickedWalkersCityNames(arrayOfClickedWalkersCities)
                ...
                */
                    window.alert(`${walker.name} services ${cityNamesForClickedWalker}`)
                }
            }
